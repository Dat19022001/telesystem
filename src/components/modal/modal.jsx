...
const Modal = () => {
  ...
  // console.log(incomingSessions)
  
  const incomingSessionsCall = Object.values(sessions).filter(
    (session) =>
      (session.sessionState === sessionStates.NEW || session.sessionState === sessionStates.PROGRESS) &&
      session.direction === sessionStates.OUTGOING
  );
  // console.log(incomingSessionsCall)
  ...
  if(incomingSessionsCall.length !== 0){
    return (
      <div
        className={`modal ${incomingSessionsCall.length !== 0 ? "modal-active" : ""}`}
      >
        {incomingSessionsCall.map((session, index) => (
          <div key={index}>
            <div className="modal-title">
              <div className="modal-name">Le Thi Soa</div>
              <div className="modal-icon">
